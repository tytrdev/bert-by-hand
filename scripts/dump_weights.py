...
def dump_layer_ref(model):
    ...
    with torch.no_grad():
        emb = bert.embeddings(input_ids=input_ids, token_type_ids=token_type_ids)
        ext = (1.0 - attention_mask[:, None, None, :].float()) * torch.finfo(torch.float32).min
        layer_out = bert.encoder.layer[0](emb, attention_mask=ext)
        if isinstance(layer_out, tuple):
            layer_out = layer_out[0]

    y = layer_out[0].numpy().astype(np.float32)
    y.tofile(REF_DIR / "layer0_out.bin")
    print(f"layer ref: shape={y.shape} |y|={np.linalg.norm(y):.4f}")
