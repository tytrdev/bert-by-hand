...
// Pipelined tensor core GEMM: 128x64 block, 4 warps each owning a 64x32 warp
// tile (4x2 fragments held in registers). A and B K-slices stream into shared
// memory with cp.async double buffering so global latency overlaps the tensor
// core math. Used for the large (batched) GEMMs.
...
void launch_matmul(const __half *A, const __half *B, __half *C, int M, int N,
  ...
  if (M >= 256 && M % PBM == 0 && N % PBN == 0 && K % PBK == 0) {
    ...
    matmul_pipe_kernel<<<grid, NW_THREADS>>>(A, B, C, M, N, K, bias);
    CUDA_CHECK_KERNEL();
    return;
  }
